export const menuItems = [
  {
    id: 1,
    title: "Shop",
    href: "/shop",
  },
  {
    id: 2,
    title: "Gallery",
    href: "/gallery",
  },
  {
    id: 3,
    title: "About",
    href: "/about",
  },
  {
    id: 4,
    title: "Contact",
    href: "/contact",
  },
];

export const shopItems = [
  {
    id: 1,
    image:
      "https://res.cloudinary.com/dtxmt9jop/image/upload/v1746329856/white-rose-baby-breadth-garland_ujhy0m.png",
    productName: "White Rose Wedding Garland",
    productDescription:
      "Woven With : Baby's Breath & White Rose Dimension: 2.5 Feet Long , Quantity : 2, No's ( 1 Pair ). Express Free Delivery & Packaging Worth Rs.600, Garland Packed In Thermocol Box With Ice Pad To Ensure Freshness.",
    productPrice: "Rs.5,995",
    availability: true,
  },
  {
    id: 2,
    image:
      "https://res.cloudinary.com/dtxmt9jop/image/upload/v1746330806/red-pink-wedding-garland_rhe5fp.png",
    productName: "Red And White Wedding Garland",
    productDescription:
      "This handcrafted wedding garland is made with vibrant red and pink flowers, symbolizing love and purity. Perfect for traditional weddings, engagements, or special ceremonies, the garland adds a touch of cultural charm to bridal and groom attire. Lightweight and comfortable, it is designed for long ceremonies and stunning photoshoots.",
    productPrice: "Rs.5275",
    availability: true,
  },
  {
    id: 3,
    image:
      "https://res.cloudinary.com/dtxmt9jop/image/upload/v1746331094/traditional-wedding-garland_t0hrdi.png",
    productName: "Traditional Wedding Garland",
    productDescription:
      "Make your special day memorable with this handcrafted Indian wedding flower garland. Designed with fresh pink and white roses, accented by soft pink touches and greenery, this garland symbolizes love and unity. Perfect for weddings, engagements, and cultural ceremonies, it enhances every moment with its vibrant beauty and fragrance. Lightweight and comfortable, it's ideal for long ceremonies.",
    productPrice: "Rs.5800",
    availability: false,
  },
  {
    id: 4,
    image:
      "https://res.cloudinary.com/dtxmt9jop/image/upload/v1746331273/jasmine-flower-garland_tzamiq.png",
    productName: "Jasmine Flower Garland",
    productDescription:
      "Celebrate your special moments with this handcrafted jasmine flower wedding garland. Featuring fresh white jasmine blooms complemented by Baby's breath, this garland symbolizes purity and love. Perfect for weddings, engagements, and cultural ceremonies, this lightweight garland is comfortable to wear for extended durations and adds a classic touch to any celebration.",
    productPrice: "Rs.6255",
    availability: true,
  },
  {
    id: 5,
    image:
      "https://res.cloudinary.com/dtxmt9jop/image/upload/v1746331494/rajni-gandha-garland_lfwgv6.png",
    productName: "Rajnigandha Garland",
    productDescription:
      "This handcrafted Rajnigandha garland is perfect for traditional weddings and cultural ceremonies. Featuring vibrant orange thread combined with fresh white flowers, this garland symbolizes joy and purity. Lightweight and easy to wear, it’s ideal for long celebrations and rituals. Perfect for weddings, engagements, or festive occasions, this garland adds a vibrant touch to your special moments.",
    productPrice: "Rs.6565",
    availability: true,
  },
];
export const homePageGridItems = [
  {
    id: 1,
    image:
      "https://plus.unsplash.com/premium_photo-1674986176795-5b8a45a38f6c?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8YXN0aGV0aWMlMjBmbG93ZXJzfGVufDB8fDB8fHww",
    text: "Asthetic Flower Arrangements",
    cssId: "box-1",
    link: "/flower-arrangement",
  },
  {
    id: 2,
    image:
      "https://plus.unsplash.com/premium_photo-1673569481316-116fb85d82d6?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8Zmxvd2VyJTIwZGVjb3JhdGlvbnN8ZW58MHx8MHx8fDA%3D",
    text: "Flowers",
    cssId: "box-2",
    link: "/shop",
  },
  {
    id: 3,
    image:
      "https://images.unsplash.com/photo-1555447405-057915b40299?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTV8fGluZGlhbiUyMHdlZGRpbmd8ZW58MHx8MHx8fDA%3D",
    text: "Memorable Weddings",
    cssId: "box-3",
    link: "/wedding",
  },
  {
    id: 4,
    image:
      "https://images.unsplash.com/photo-1635866319966-00eef422cf64?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8Ym91cWV0c3xlbnwwfHwwfHx8MA%3D%3D",
    text: "Beautiful Bouquets",
    cssId: "box-4",
    link: "/bouquet",
  },
  {
    id: 5,
    image:
      "https://images.unsplash.com/photo-1556125574-d7f27ec36a06?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8ZXZlbnRzfGVufDB8fDB8fHww",
    text: "Events and Celebrations",
    cssId: "box-5",
    link: "/events-celebrations",
  },
  {
    id: 6,
    image:
      "https://images.unsplash.com/photo-1556125574-d7f27ec36a06?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8ZXZlbnRzfGVufDB8fDB8fHww",
    text: "Events and Celebrations",
    cssId: "box-6",
    link: "/event",
  },
];
export const blogSectionItems = [
  {
    id: 1,
    image:
      "https://images.unsplash.com/photo-1556125574-d7f27ec36a06?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8ZXZlbnRzfGVufDB8fDB8fHww",
    text: "lily",
    title: "The Timeless Beauty of Flowers: Nature’s Finest Masterpiece",
  },
  {
    id: 2,
    image:
      "https://plus.unsplash.com/premium_photo-1682090795093-9b9434a455ec?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NXx8aW5kaWFuJTIwd2VkZGluZ3xlbnwwfHwwfHx8MA%3D%3D",
    text: "lily",
    title: "Wedding Flowers: A Celebration of Love and Beauty",
  },
  {
    id: 3,
    image:
      "https://images.unsplash.com/photo-1556125574-d7f27ec36a06?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8ZXZlbnRzfGVufDB8fDB8fHww",
    text: "lily",
    title: "functions of flowers",
  },
];
export const FlowerArrangementgalleryItems: Array<{
  image: string;
  alt: string;
  subtitle: string;
  title: string;
  description: string;
  width: number;
  height: number;
}> = [
  {
    image:
      "https://i.pinimg.com/enabled_lo_mid/736x/45/32/0c/45320c84b8f91017a703b663ec35851d.jpg",
    alt: "A beautiful starry night sky with shooting stars",
    subtitle: "THE SUBTITLE",
    title: "Shooting Stars",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 600,
    height: 360,
  },
  {
    image:
      "https://i.pinimg.com/enabled_lo_mid/736x/17/1e/0b/171e0ba9a90826528cd8c4d924dad443.jpg",
    alt: "Abstract artistic scene with vibrant colors",
    subtitle: "THE SUBTITLE",
    title: "The Catalyzer",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 601,
    height: 361,
  },
  {
    image:
      "https://i.pinimg.com/736x/be/bc/85/bebc8565653a7b10da36520e6fb024da.jpg",
    alt: "An artistic rendition of a cinematic scene",
    subtitle: "THE SUBTITLE",
    title: "The 400 Blows",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 603,
    height: 363,
  },
  {
    image:
      "https://i.pinimg.com/enabled_lo_mid/736x/87/a1/cd/87a1cdfa4206cf02ae18d8961ce9faed.jpg",
    alt: "A tranquil scene featuring the planet Neptune",
    subtitle: "THE SUBTITLE",
    title: "Neptune",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 602,
    height: 362,
  },
  {
    image:
      "https://i.pinimg.com/enabled_lo_mid/736x/ca/2d/48/ca2d48790cf8a42706d4a48a3a5bd429.jpg",
    alt: "Portrait of a character named Holden Caulfield",
    subtitle: "THE SUBTITLE",
    title: "Holden Caulfield",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 605,
    height: 365,
  },
  {
    image:
      "https://i.pinimg.com/736x/24/95/f4/2495f46608d5d946c6ffd1fb6d6a17be.jpg",
    alt: "Creative artwork titled 'Alper Kamu'",
    subtitle: "THE SUBTITLE",
    title: "Alper Kamu",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 606,
    height: 366,
  },
];

export const flowerArrangementsImages = [
  "https://i.pinimg.com/enabled_lo_mid/736x/17/1e/0b/171e0ba9a90826528cd8c4d924dad443.jpg",
  "https://i.pinimg.com/736x/be/bc/85/bebc8565653a7b10da36520e6fb024da.jpg",
  "https://i.pinimg.com/enabled_lo_mid/736x/87/a1/cd/87a1cdfa4206cf02ae18d8961ce9faed.jpg",
  "https://i.pinimg.com/enabled_lo_mid/736x/ca/2d/48/ca2d48790cf8a42706d4a48a3a5bd429.jpg",
  "https://i.pinimg.com/736x/24/95/f4/2495f46608d5d946c6ffd1fb6d6a17be.jpg",
];

export const blogdetailItems = [
  {
    titleUrl: "flower-blog-1",
    id: 1,
    image:
      "https://images.unsplash.com/photo-1556125574-d7f27ec36a06?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8ZXZlbnRzfGVufDB8fDB8fHww",
    title: "The Timeless Beauty of Flowers: Nature’s Finest Masterpiece",
    para1:
      "Flowers, with their vivid colors, intricate patterns, and intoxicating fragrances, are among nature’s most exquisite creations. These delicate marvels not only captivate the eye but also hold profound cultural, emotional, and ecological significance. From symbolizing love to supporting entire ecosystems, flowers are an integral part of life on Earth.",
    heading2: "The Allure of Flowers",
    para2:
      "Flowers are a universal language. They transcend borders, languages, and cultures, often used to convey emotions that words cannot fully capture. Whether it’s the elegance of a rose, the cheerful simplicity of a daisy, or the exotic allure of an orchid, flowers appeal to our senses in unique and powerful ways.",
    heading3: "Colors and Fragrances",
    para3:
      "Flowers come in every color imaginable, each shade evoking specific emotions:",
    list: [
      "Red for passion and love.",
      "Yellow for joy and friendship.",
      "White for purity and peace.",
      "Purple for luxury and royalty.",
    ],
    para4:
      "Their fragrances range from subtle and sweet to rich and spicy, often acting as nature’s way of attracting pollinators like bees, butterflies, and birds.",
    heading4: "Shapes and Patterns",
    para5:
      "The variety in flower shapes and patterns is astonishing. From the geometric precision of sunflowers to the chaotic beauty of wildflowers, each flower tells a story of evolution and adaptation. These features often serve a dual purpose: enhancing their beauty and aiding in pollination by guiding insects to their nectar.",
  },
  {
    titleUrl: "flower-blog-2",
    id: 2,
    image:
      "https://images.unsplash.com/photo-1556125574-d7f27ec36a06?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8ZXZlbnRzfGVufDB8fDB8fHww",
    title: "The Timeless Beauty of Flowers: Nature’s Finest Masterpiece",
    para1:
      "Flowers, with their vivid colors, intricate patterns, and intoxicating fragrances, are among nature’s most exquisite creations. These delicate marvels not only captivate the eye but also hold profound cultural, emotional, and ecological significance. From symbolizing love to supporting entire ecosystems, flowers are an integral part of life on Earth.",
    heading2: "The Allure of Flowers",
    para2:
      "Flowers are a universal language. They transcend borders, languages, and cultures, often used to convey emotions that words cannot fully capture. Whether it’s the elegance of a rose, the cheerful simplicity of a daisy, or the exotic allure of an orchid, flowers appeal to our senses in unique and powerful ways.",
    heading3: "Colors and Fragrances",
    para3:
      "Flowers come in every color imaginable, each shade evoking specific emotions:",
    list: [
      "Red for passion and love.",
      "Yellow for joy and friendship.",
      "White for purity and peace.",
      "Purple for luxury and royalty.",
    ],
    para4:
      "Their fragrances range from subtle and sweet to rich and spicy, often acting as nature’s way of attracting pollinators like bees, butterflies, and birds.",
    heading4: "Shapes and Patterns",
    para5:
      "The variety in flower shapes and patterns is astonishing. From the geometric precision of sunflowers to the chaotic beauty of wildflowers, each flower tells a story of evolution and adaptation. These features often serve a dual purpose: enhancing their beauty and aiding in pollination by guiding insects to their nectar.",
  },
  {
    titleUrl: "flower-blog-3",
    id: 3,
    image:
      "https://images.unsplash.com/photo-1556125574-d7f27ec36a06?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8ZXZlbnRzfGVufDB8fDB8fHww",
    title: "The Timeless Beauty of Flowers: Nature’s Finest Masterpiece",
    para1:
      "Flowers, with their vivid colors, intricate patterns, and intoxicating fragrances, are among nature’s most exquisite creations. These delicate marvels not only captivate the eye but also hold profound cultural, emotional, and ecological significance. From symbolizing love to supporting entire ecosystems, flowers are an integral part of life on Earth.",
    heading2: "The Allure of Flowers",
    para2:
      "Flowers are a universal language. They transcend borders, languages, and cultures, often used to convey emotions that words cannot fully capture. Whether it’s the elegance of a rose, the cheerful simplicity of a daisy, or the exotic allure of an orchid, flowers appeal to our senses in unique and powerful ways.",
    heading3: "Colors and Fragrances",
    para3:
      "Flowers come in every color imaginable, each shade evoking specific emotions:",
    list: [
      "Red for passion and love.",
      "Yellow for joy and friendship.",
      "White for purity and peace.",
      "Purple for luxury and royalty.",
    ],
    para4:
      "Their fragrances range from subtle and sweet to rich and spicy, often acting as nature’s way of attracting pollinators like bees, butterflies, and birds.",
    heading4: "Shapes and Patterns",
    para5:
      "The variety in flower shapes and patterns is astonishing. From the geometric precision of sunflowers to the chaotic beauty of wildflowers, each flower tells a story of evolution and adaptation. These features often serve a dual purpose: enhancing their beauty and aiding in pollination by guiding insects to their nectar.",
  },
];

export const weddingItems: Array<{
  image: string;
  alt: string;
  subtitle: string;
  title: string;
  description: string;
  width: number;
  height: number;
}> = [
  {
    image:
      "https://plus.unsplash.com/premium_photo-1682090795093-9b9434a455ec?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NXx8aW5kaWFuJTIwd2VkZGluZ3xlbnwwfHwwfHx8MA%3D%3D",
    alt: "Portrait of a character named Holden Caulfield",
    subtitle: "THE SUBTITLE",
    title: "Wedding Flowers: A Celebration of Love and Beauty",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 602,
    height: 362,
  },
  {
    image:
      "https://images.unsplash.com/photo-1665960213530-3fb10da1f25e?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NTZ8fGluZGlhbiUyMHdlZGRpbmd8ZW58MHx8MHx8fDA%3D",
    alt: "Portrait of a character named Holden Caulfield",
    subtitle: "THE SUBTITLE",
    title: "Wedding Flowers: A Celebration of Love and Beauty",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 602,
    height: 362,
  },
  {
    image:
      "https://images.unsplash.com/photo-1698460916760-b9e3bba58914?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    alt: "Portrait of a character named Holden Caulfield",
    subtitle: "THE SUBTITLE",
    title: "Wedding Flowers: A Celebration of Love and Beauty",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 602,
    height: 362,
  },
  {
    image:
      "https://images.unsplash.com/photo-1656814516314-65483701d219?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTAzfHxpbmRpYW4lMjB3ZWRkaW5nfGVufDB8fDB8fHww",
    alt: "Portrait of a character named Holden Caulfield",
    subtitle: "THE SUBTITLE",
    title: "Wedding Flowers: A Celebration of Love and Beauty",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 602,
    height: 362,
  },
];

export const weddingImages = [
  "https://plus.unsplash.com/premium_photo-1682090795093-9b9434a455ec?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NXx8aW5kaWFuJTIwd2VkZGluZ3xlbnwwfHwwfHx8MA%3D%3D",
  "https://images.unsplash.com/photo-1665960213530-3fb10da1f25e?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NTZ8fGluZGlhbiUyMHdlZGRpbmd8ZW58MHx8MHx8fDA%3D",
  "https://images.unsplash.com/photo-1698460916760-b9e3bba58914?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
  "https://images.unsplash.com/photo-1656814516314-65483701d219?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTAzfHxpbmRpYW4lMjB3ZWRkaW5nfGVufDB8fDB8fHww",
];
export const eventsAndCelebrationsItems: Array<{
  image: string;
  alt: string;
  subtitle: string;
  title: string;
  description: string;
  width: number;
  height: number;
}> = [
  {
    image:
      "https://images.unsplash.com/photo-1744891470331-c660191721b5?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8b2ZmaWNlJTIwcGFydGllcyUyMGltYWdlcyUyMGluJTIwSW5kaWF8ZW58MHx8MHx8fDI%3D",
    alt: "office parties",
    subtitle: "office parties",
    title: "Office Parties",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 602,
    height: 362,
  },
  {
    image:
      "https://images.unsplash.com/photo-1636256373111-cddaa1470363?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTF8fGZpcnN0JTIwYmlydGhkYXl8ZW58MHx8MHx8fDI%3D",
    alt: "birthday",
    subtitle: "birthday",
    title: "Birthday Parties",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 602,
    height: 362,
  },
  {
    image:
      "https://images.unsplash.com/photo-1656435572228-0e3a000c4a8e?q=80&w=1931&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    alt: "Bride to be",
    subtitle: "Bride to be",
    title: "Bridal Showers",
    description:
      "Photo booth fam kinfolk cold-pressed sriracha leggings jianbing microdosing tousled waistcoat.",
    width: 602,
    height: 362,
  },
];
export const eventsAndCelebrationsImages = [
  "https://images.unsplash.com/photo-1744891470331-c660191721b5?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8b2ZmaWNlJTIwcGFydGllcyUyMGltYWdlcyUyMGluJTIwSW5kaWF8ZW58MHx8MHx8fDI%3D",
  "https://images.unsplash.com/photo-1636256373111-cddaa1470363?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTF8fGZpcnN0JTIwYmlydGhkYXl8ZW58MHx8MHx8fDI%3D",
  "https://images.unsplash.com/photo-1656435572228-0e3a000c4a8e?q=80&w=1931&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
];
